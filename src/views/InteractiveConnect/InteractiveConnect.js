...
export default class InteractiveConnect extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  handleTaskMessage = async ({ detail }) => {
    // Look for the shell.html artifact if the message originates from the artifact create exchange
    if (detail.exchange === this.props.queueEvents.artifactCreated().exchange) {
      this.setState({
    ...
    return (
      <div>
        ...
        <div className="text-center">
          ...
          <div onClick={this.openDisplay} className={connectLinkButton}>
            <span className="fa-stack fa-5x">
              <i className="fa fa-circle fa-stack-2x" />
