...
import { Table, Label, ButtonToolbar, DropdownButton, MenuItem } from 'react-bootstrap';
            ...
            {tasksToRender.length ?
              tasksToRender.map((task, index) => (
                <tr key={`inspector-task-row-${index}`}>
                  <td><Label bsSize="sm" bsStyle={labels[task.status.state]}>{task.status.state}</Label></td>
                  <td>
                    <Link to={`/groups/${taskGroupId}/tasks/${task.status.taskId}/details`} replace>
