...
export default class WorkerTypeTable extends React.PureComponent {
  ...
  setSelected = workerType => this.props.history
    .replace(`/aws-provisioner-staging/${workerType}/${this.props.currentTab}`);

  handleTabChange = tab => this.props.history.replace(`/aws-provisioner-staging/${this.props.workerType}/${tab}`);
